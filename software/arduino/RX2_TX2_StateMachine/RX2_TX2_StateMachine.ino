 ...
 * Controller IC with an Arduino, using a state machine to
 * cycle through a few of the various functions
 *
...
void pulseStatus() {
  // Rapidly toggle the status output pin one time
  digitalWrite(STATUS_PIN, HIGH);
  delay(50);  
  digitalWrite(STATUS_PIN, LOW);  
  delay(50);  
...
void doStateMachine() {
  // Advance state machine once per interval passed
  unsigned long currMillis = millis();

  if (currMillis - prevMillis >= smInterval) {
    prevMillis = currMillis;

    state++;
  }

  // Do the assigned function for each state 
  switch (state) {
    // State 1 = Send forward code signal
    case 1:
      doFunction(W1FX_FORWARD);
      break;

    // State 3 = Send left code signal
    case 3:  
      doFunction(W1FX_LEFT);
      break;

    // State 5 = Send backward code signal
    case 5:  
      doFunction(W1FX_BACKWARD);
      break;

    // State 7 = Send right code signal
    case 7:  
      doFunction(W1FX_RIGHT);
      break;

    // Default state (plus states 0,2,4,6) to "pause" output
    default:
      pulseStatus();

      // Only 8 states in machine, reset to beginning as needed
      if (state > 7) {
        state = 0;
      }
  }  
...
void loop() {
  // Run the state machine functions
  doStateMachine();
}
